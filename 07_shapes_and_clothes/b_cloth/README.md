...
A cloth surface is modeled as a mesh whose vertices are initially placed on a regular grid.
...
- A set of GUI-based controllers on the time-scale multiplier applied to $h$, stiffness, damping, mass, and wind force. These parameters are available inside the code (only time scale is used in the initial code).
- The option to deactivate the cloth texture and display the wireframe, which may be helpfull for debugging.
...
- Add first only the forces of structural springs and make sure that you get a plausible behavior before adding more forces. Using wireframe mode with slow time step and coarse mesh (few particles) can help debugging if something is wrong.
- Then add forces modeled by shearing and bending springs.

*Reminders*:

Consider a spring of stiffness $K$ and rest length $L_0$​ attached between a point $p_i$​ and $p_j$​. The force exerted by the spring on the point $p_i$​ is:
...
Implement the *action of the wind* or wind force. How can you model such force? (check `simulation.parameters`)
