...
In the current state, the code encodes a velocity grid (the velocity of the fluid) and a density grid (the color/image which is visualized). The values of these grids are only advected through time, and your objective is to understand the three steps of the stable fluids method, namely 
- Diffusion
- Advection
- Projection of the velocity to a divergence free vector field
...
To ensure stable advection, backtracing can be used in setting the value $f(p,t)$ where $p$ is a grid cell position, and $t$ the current time from the interpolated value of $f$ at previous time step at position $f(p-\Delta tv(p,t), t -\Delta t).$
...
## ${\color{cyan}\bf{TODO}}$: Diffusion

The function diffuse is set to solve one step of the equation $\frac{\partial f}{\partial t} = \mu \Delta f$, where $f$ can be either the velocity, or a density. To handle this generic input, the function is encoded to receive a 2D grid of template values. The function is also defined in the header file for this reason.
...
Implement the numerical solution of this equation using Gauss Seidel iterations (consider around 15 iterations). The function to complete uses template arguments and is defined in the file `simulation.hpp`.
...
## ${\color{cyan}\bf{TODO}}$: Divergence Free Projection
...
2. Compute a gradient field $q$ satisfying Poisson equation $\nabla^2 q = div(v)$
3. Compute divergence free vector field as $v=v_0 - \nabla q$

Similarly to the divergence computation, the Poisson equation is solved using a stable implicit numerical scheme, typically using the Gauss Seidel iterations. Between each of these steps, and at each iteration of the Gauss Seidel, boundaries of the different quantities can be reset using the `set_boundary` function.
